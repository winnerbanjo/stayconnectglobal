    ...
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
          scrolled
            ? 'bg-[#111111]/95 backdrop-blur-xl border-b border-[#2C2B29] py-5 shadow-2xl'
            : isHome
            ? 'bg-gradient-to-b from-black/90 via-black/40 to-transparent py-7'
            : 'bg-[#111111] py-6 border-b border-[#2C2B29]'
        }`}
      >
        <div className="max-w-7xl mx-auto px-8 lg:px-16 flex items-center justify-between">
          {/* Brand Identity */}
          <Link href="/" className="group flex items-center gap-4">
            <div className="w-10 h-10 rounded-full bg-[#1A1918] border border-[#C6A15B]/50 flex items-center justify-center transition-all duration-300 group-hover:border-[#C6A15B] group-hover:scale-105">
              <span className="font-serif text-[#C6A15B] text-lg font-semibold tracking-wider">SC</span>
            </div>
            <div className="flex flex-col justify-center">
              <span className="font-serif text-xl tracking-[0.3em] text-white uppercase font-normal leading-none">
                STAY CONNECT
              </span>
              <span className="text-[9px] tracking-[0.4em] text-[#C6A15B] uppercase font-light mt-1">
                Hotels & Suites • Lekki
              </span>
            </div>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-6">
            ...
            >
              ...
              <span className="hidden xl:inline">+234 803 123 4567</span>
            </a>
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-40 bg-[#111111] pt-28 px-8 pb-12 flex flex-col justify-between overflow-y-auto"
          >
            <div className="flex flex-col gap-6">
              ...
              {navLinks.map((link) => (
                <Link
                  ...
                  className="font-serif text-2xl text-white hover:text-[#C6A15B] transition-colors"
                >
                  {link.name}
                </Link>
              ))}
            ...
            <div className="pt-8 border-t border-[#2C2B29] flex flex-col gap-4">
              <Link
                href="/book"
                onClick={() => setMobileMenuOpen(false)}
                className="w-full text-center py-3.5 rounded bg-[#C6A15B] text-[#111111] font-medium text-xs uppercase tracking-widest shadow-xl"
              >
                Book Your Stay
              </Link>
              <div className="text-center text-xs text-neutral-500 tracking-wider font-light">
