...
export default function ImageUploadStatus({ progress, uploading }: { progress: ImageUploadProgress | null; uploading: boolean }) {
  ...
  return <div role="status" aria-live="polite" className="rounded-lg border border-[#C6A15B]/40 p-3 space-y-2 text-xs">
    <div className="flex items-center gap-2">
      ...
      <span>{uploading ? `Uploading photo ${Math.min(progress.completed + 1, progress.total)} of ${progress.total}…` : complete ? `${progress.total} ${progress.total === 1 ? "photo uploaded" : "photos uploaded"}` : `Upload stopped. ${progress.completed} of ${progress.total} photos uploaded.`}</span>
    </div>
