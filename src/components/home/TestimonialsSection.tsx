      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12 space-y-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {INITIAL_REVIEWS.map((rev) => (
            ...
            >
              <Quote className="absolute top-6 right-6 w-10 h-10 text-[#00AEEF]/15" />

              <div className="space-y-4">
                <div className="flex items-center gap-1 text-[#00AEEF]">
                  {[...Array(5)].map((_, i) => (
                    <Star key={i} className="w-4 h-4 fill-[#00AEEF]" />
                  ))}
              ...
              <div className="pt-6 border-t border-[#E8E5DF] flex items-center justify-between">
                <div className="flex items-center gap-3">
                  {rev.guestAvatar ? (
                    <div className="relative w-10 h-10 rounded-full overflow-hidden border border-[#00AEEF]">
                      <Image src={rev.guestAvatar} alt={rev.guestName} fill className="object-cover" />
                    </div>
                  ) : (
                    <div className="w-10 h-10 rounded-full bg-[#111111] text-[#00AEEF] flex items-center justify-center font-serif text-sm">
