...
function OperationsContent({ section }: { section: Section }) {
  ...
  return <div className="min-h-screen bg-[#111111] text-white">
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 sm:px-8 space-y-8">
      <nav className="flex flex-wrap gap-4 text-sm text-[#C6A15B]" aria-label="Admin navigation">{links.map(([path, label]) => <Link key={path} href={`/admin${path ? `/${path}` : ""}`} aria-current={path === section ? "page" : undefined}>{label}</Link>)}</nav>
      <header className="flex items-center justify-between gap-4"><h1 className="font-serif text-3xl">{config.title}</h1><button disabled={loading || saving} onClick={() => { setError(""); setForm({ ...config.defaults }); }} className="bg-[#C6A15B] text-black rounded-lg px-5 py-3">Add record</button></header>
      {error && <div role="alert" className="text-rose-300 border border-rose-800 rounded-lg p-4">{error} {!form && <button onClick={load} className="underline ml-4">Retry</button>}</div>}
      {loading ? <p role="status">Loading saved records…</p> : records.length === 0 ? <p className="text-neutral-400">No saved records yet. Add your first {section === "fleet" ? "vehicle" : section === "dining" ? "menu item" : "room cleaning record"}.</p> : <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-5">{records.map(record => <article key={record.id} className="bg-[#1A1918] border border-[#2C2B29] rounded-xl p-6 space-y-3">
        {config.fields.map(field => <div key={field.key}><div className="text-xs text-neutral-400">{field.label}</div><div className="break-words">{field.type === "checkbox" ? record[field.key] ? "Yes" : "No" : field.type === "room" ? rooms.find(room => room.id === record.roomId)?.name || "Room no longer available" : field.type === "number" ? `₦${Number(record[field.key]).toLocaleString()}` : record[field.key] || "Not assigned"}</div></div>)}
        {section === "housekeeping" && <p className="text-xs text-neutral-400">Last inspected: {record.lastCleaned ? new Date(record.lastCleaned).toLocaleString() : "Not recorded"}</p>}
        <button onClick={() => { setError(""); setForm({ ...record }); }} className="text-[#C6A15B] underline">Edit record</button>
        <button disabled={saving} onClick={() => remove(record.id)} className="text-rose-300 underline ml-5">Delete record</button>
      </article>)}</div>}
      {form && <section className="bg-[#1A1918] border border-[#C6A15B]/40 rounded-xl p-6 max-w-2xl" aria-label="Record editor"><h2 className="text-xl mb-5">{form.id ? "Edit" : "Add"} record</h2><form onSubmit={submit} className="space-y-4">
        {config.fields.map(field => <label key={field.key} className="block text-sm">{field.label}{field.choices || field.type === "room" ? <select required value={form[field.key]} onChange={e => setForm({ ...form, [field.key]: e.target.value })} className="block w-full bg-[#111111] border border-neutral-700 rounded p-3 mt-1">{field.type === "room" ? <><option value="">Choose a room type</option>{rooms.map(room => <option key={room.id} value={room.id}>{room.name}</option>)}</> : field.choices?.map(value => <option key={value}>{value}</option>)}</select> : <input required={!field.optional && field.type !== "checkbox"} type={field.type || "text"} min={field.type === "number" ? 0 : undefined} step={field.type === "number" ? "0.01" : undefined} checked={field.type === "checkbox" ? form[field.key] : undefined} value={field.type === "checkbox" ? undefined : form[field.key]} onChange={e => setForm({ ...form, [field.key]: field.type === "checkbox" ? e.target.checked : e.target.value })} className={field.type === "checkbox" ? "ml-3" : "block w-full bg-[#111111] border border-neutral-700 rounded p-3 mt-1"} />}</label>)}
        <div className="flex gap-4"><button disabled={saving} className="bg-[#C6A15B] text-black rounded px-5 py-3">{saving ? "Saving…" : "Save record"}</button><button type="button" disabled={saving} onClick={() => setForm(null)}>Cancel</button></div>
      </form></section>}
    </div>
  </div>;
}
